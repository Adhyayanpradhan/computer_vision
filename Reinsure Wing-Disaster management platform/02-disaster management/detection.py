...
def people():
    ...
    vs = FileVideoStream("v1.mp4").start()
    ...
    while True:
        ...
        if len(idxs) > 0:
            ...
            cv2.putText(frame, "Estimated Teams to be deployed: " + str(int(len(idxs)-(len(idxs)/2))), (0,frame.shape[0] -30), cv2.FONT_HERSHEY_TRIPLEX, 0.5 , (255,255,255), 1)
            
            count=len(idxs)
            out=cv2.imwrite("object%d.jpg"%count,frame)
            now = datetime.now()
            current_time = now.strftime("%H:%M:%S")
            cv2.putText(frame, "Current time of saving the object detected " + current_time, (0,frame.shape[0] -10), cv2.FONT_HERSHEY_TRIPLEX, 0.5,  (255,255,255), 1)
            cv2.putText(frame, "Geo location: " + str('latitude:21.4971,longitude:83.9038'), (0,frame.shape[0] -70), cv2.FONT_HERSHEY_TRIPLEX, 0.5 , (255,255,255), 1)
