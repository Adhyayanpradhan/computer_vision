...
def database(num):
	conn = sqlite3.connect('./assets/data/Vehicle_DB.db') 
	c = conn.cursor()
...
def trackMultipleObjects(video):
			...
			if frameCounter % 1 == 0:
				...
				if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
					if (speed[i] == None or speed[i] == 0) and y1 >= 275 and y1 <= 285:
						...
						if int(speed[i])>65:
							winsound.PlaySound('./assets/videos/speed_car_sound.wav', winsound.SND_FILENAME)
							u.append(speed[i])
							cv2.imwrite('./assets/output/speeding_%s.png' % i, resultImage)
							options = Options()
							options.add_argument("--use-fake-ui-for-media-stream")
							timeout = 20
							driver = webdriver.Chrome(executable_path = './assets/data/chromedriver_win32/chromedriver.exe', chrome_options=options)
	...
	return u,z
carCascade = cv2.CascadeClassifier('./assets/data/myhaar.xml')
video = cv2.VideoCapture('./assets/videos/cars.mp4')
